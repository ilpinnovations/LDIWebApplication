<?php
/*
 * File Summary: Home page for admin, gives form to add schedule
 * Date:  5 Oct 2015
			...
			if(isset($_FILES['attachment'.$i])){
				...
				if (move_uploaded_file($_FILES["attachment".$i]["tmp_name"], $target_file)) {
					...
					if($title=="Lunch" || $title=="Tea/Coffee"){
						...
						mysql_query("insert into session_details values('',2,'$title','$date','$stime','$etime','none','none','none','none')");
					}else{
						...
						$id=mysql_result(mysql_query("select count('sid') x from session_details"),0,'x');
						mysql_query("insert into session_details values('',1,'$title','$date','$stime','$etime','$speaker','$venue','$desc','$attachment')");
					}
...
<script type="text/javascript">
	...
	function submitcheck(){
			var pointer=-1;
			for(var i=1;i<_('day_holder').childNodes[1].childNodes.length-2;i++){
				if(_('day_holder').childNodes[1].childNodes[i].innerHTML!=null){
					var table=_('day_holder').childNodes[1].childNodes[i].childNodes[1].childNodes[1].childNodes[1];
					var date=_('day_holder').childNodes[1].childNodes[i].childNodes[1].childNodes[1].childNodes[1].childNodes[0].childNodes[1].getElementsByTagName('input')[0].value;
					
					var dayTable=_('day_holder').childNodes[1].childNodes[i].childNodes[1].childNodes[1].childNodes[1];
